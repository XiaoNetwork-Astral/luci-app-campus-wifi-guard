...
设置页分为「基础设置」「上联网络」「故障恢复」「隐私与计划」「高级设置」。先在基础设置中填写账号与公网检测地址，再配置上联；重试次数、切换策略和门户参数位于各自分组。不适用的参数会收起并保留已保存的值。状态页显示服务概况、每条上联和最近事件；会中断连接的操作放在各上联的「更多操作」中，暂停后的恢复按钮统一显示在服务概况中。
